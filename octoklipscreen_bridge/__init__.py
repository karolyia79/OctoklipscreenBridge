...
Sends serial logs, status, printer state/temps, profiles and job data via MQTT to CYD display
Includes server-side statistics and REST API management with unique file names.
"""
...
class OctoklipscreenBridgePlugin(octoprint.plugin.StartupPlugin,
    ...
    """
    OctoPrint plugin for bridging serial logs, printer info to MQTT,
    and managing server-side print statistics with dedicated storage.
    """
            ...
            try:
                ...
                printer_data = {
                    ...
                    "state": {
                        "error": "",
                        "flags": {
                            "cancelling": False, "closedOrError": False, "error": False,
                            "finishing": False, "operational": is_operational, "paused": is_paused,
                            "pausing": False, "printing": is_printing,
    ...
    # =========================================================
    # --- SZERVER OLDALI STATISZTIKA MODUL (Egyedi fájlnevek) ---
    # =========================================================
    ...
    def _determine_material(self, nozzle_target, bed_target):
        ...
        for rule in cfg.get("materials", []):
            if (rule.get("n_min", 0) <= nozzle_target <= rule.get("n_max", 0) and
                rule.get("b_min", 0) <= bed_target <= rule.get("b_max", 0)):
    ...
    def _handle_print_started(self, payload):
        ...
        self._current_material = self._determine_material(nozzle_target, bed_target)
    ...
    def _handle_print_finished(self, success):
        ...
        try:
            with open(csv_path, "a", encoding="utf-8") as f:
                f.write(f"{self._active_system},{self._current_material},{duration_seconds},{1 if success else 0},{current_cost:.2f}\n")
            logger.info(f"[STAT] Nyomtatás rögzítve -> Idő: {duration_seconds}mp, Siker: {success}, Költség: {current_cost:.2f}")
        except Exception as e:
    ...
    def _aggregate_stats(self):
        cfg = self._load_config()
        stats = {
            "totalTimeOcto": 0, "totalTimeKlipper": 0,
            "successCountOcto": 0, "failCountOcto": 0,
            "successCountKlipper": 0, "failCountKlipper": 0,
            "totalCost": 0.0, "failCost": 0.0
        }
        mat_times = {}
                    ...
                    for line in lines[1:]:
                        ...
                        if len(parts) >= 5:
                            ...
                            if sys == "OctoPrint":
                                ...
                                if succ: stats["successCountOcto"] += 1
                                else: stats["failCountOcto"] += 1
                            elif sys == "Klipper":
                                stats["totalTimeKlipper"] += dur
                                if succ: stats["successCountKlipper"] += 1
                                else: stats["failCountKlipper"] += 1
    ...
    def get_stats_data(self):
        ...
        if is_printing:
            return flask.jsonify({"status": "printing"})
